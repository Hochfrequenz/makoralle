...
from makoralle.notation import deadline_tag, states_a_hard_date

#: The notation moved to :mod:`makoralle.notation` (makoralle#65) — this module is named
#: after a renderer that no longer runs, and the tag it draws is not WSD-specific. The old
#: private name is kept because makorele's ``test_deadline_corpus`` and
#: ``test_p12_deadline_dehyphenation`` import it from here, and a rename that turns another
#: repo's CI red on the release that carries it is not a rename anyone can review. Drop it
#: once makorele imports :func:`makoralle.notation.deadline_tag`.
_deadline_tag = deadline_tag
...
def _unverzueglich_sentence_beyond_the_tag(rule: DeadlineRule) -> str:
    ...
    if deadline is None:
        return ""
    if states_a_hard_date(deadline) and not _holds_a_prose_anchor(deadline):
        return ""
...
def _holds_a_prose_anchor(deadline: Deadline) -> bool:
    ...
    :func:`makoralle.notation._step_anchor_tag` cannot put on a label. Without this a rule naming both a prose
